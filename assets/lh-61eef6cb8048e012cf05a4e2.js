...
var current_variant = null;
 ...
 };						cb.bind($('section[data-section-id="3XtP0oA5gW"] include[data-name="product-form"][data-id="2"]').get(0))(jQuery);;var cb  = function($) {
 ...
 };						cb.bind($('section[data-section-id="3XtP0oA5gW"] include[data-name="tabs"][data-id="4"]').get(0))(jQuery);;var cb  = function($) {
...
$_this.find('.count_time').on('time.elapsed', function () {
 ...
 };						cb.bind($('section[data-section-id="p0zzqrkw91g"] include[data-name="countdown"][data-id="4"]').get(0))(jQuery);;var cb  = function($) {

var _this = jQuery(this),
		$this = _this.closest('.lh-product-single'),
		slide = $this.find('.block-slide-lg'),
		thumb = $this.find('.block-slide-sm'),
		arr_slide = [];
var id = $this.data('pid'),money_format =decodeURIComponent($this.data('moneyFormat')).replace(/\+/g,' ');
var current_variant = null;
	if(window.LAYOUTHUB_LIVE){
	...
	function ProductOptionsInit() {
	    ...
	    function selectCallback(variant, selector) {
          current_variant = variant;
					...
					var status = _this.find('.lh-wrap-stock'),
							sku    = $this.find('.lh-sku span'),
	            ...
	            _this.find('.lh-cart-button-group').addClass('disabled');
							sku.text('N/A');
							status.removeClass('lh-available-true').find('.lh-product-status').attr("data-lhi","trans_unavai").text(section.settings.trans_unavai);
	            ...
	            return;
					...
					if(progress.length && quantity){
						...
						if(is_show){
							var text_progress = section.settings.label_progress;
							progress.find('.lh-text-bar span').text('(' + text_progress.replace("{left}",left) +  ')');
							progress.find('.lh-progress-bar').css('width',Math.round(left * 100 / total) + "%");
						}
					}else progress.removeClass("lh-show");
					if(quantity < 1 && policy == 'continue'){
					...
					$this.find('.lh-content-summary').attr('data-hidden', !isSale);
					status.addClass('lh-available-true').find('.lh-product-status').attr("data-lhi","trans_outstock").text(section.settings.trans_outstock);
	        $this.find('.lh-cart-button-group').removeClass('disabled');
	        $this.find('.lh-price').css('display','block').html(window.HUB.formatMoney(variant.price,money_format));
					sku.text(variant.sku ? variant.sku: "N/A");
	        if (isSale) {
							label_sale.css('display','block');
	            $this.find('.lh-compare-at-price').css('display','block').html(window.HUB.formatMoney(variant.compare_at_price,money_format));
	        } else {
	        ...
	        if (!variant.available) {
	            _this.find('.lh-cart-button-group').addClass('disabled');
							sku.text('N/A');
							status.removeClass('lh-available-true');
	        }
	    ...
	    $this.find('.lh-swatch-select').on('click', function() {
	        var _this = $(this);
					if(!_this.hasClass('active') && !$this.hasClass('lh-onchange-variant')){
						$this.addClass('lh-onchange-variant');
						var is_swatch_main = _this.closest('.lh-swatch-images').length > 0;
						if(is_swatch_main)
							filterSlide(_this.data('value'));
		        _this.siblings('li').removeClass('active');
		        _this.addClass('active');
		        var index = _this.closest('.swatch-index').attr('data-option-index');
		        $this.find('select#product-select-' + id + '-option-' + index).val(_this.data('value')).trigger('change');
						_this.closest('.lh-swatch-images').find('.meta span').text(_this.data('value'));
					}
	    });
	}
	function getDataSlide(){
		data_slide = slide.find('.items-img').clone();
	  data_thumb = thumb.find('.items-img').clone();
	  arr_slide[0] = data_slide;
	  arr_slide[1] = data_thumb;
	}
	function initSlide() {
	    data_slide = {
	      speed : 500,
		...
		if (thumb.hasClass('lh-reinit')){
			var $thumb = new Swiper(thumb, jQuery.extend({
		    slidesPerView: 'auto',
		    spaceBetween: 8,
				breakpoints: {
		    0: {
			...
			thumb.removeClass('lh-reinit')
		}
		if (slide.hasClass('lh-reinit')){
			var $slide = new Swiper(slide, jQuery.extend({
		    slidesPerView: 1,
		    navigation: {
		        nextEl: '.lh-slide-lg-nav .swiper-button-next',
		        prevEl: '.lh-slide-lg-nav .swiper-button-prev',
		    ...
		    on : {
					slideChange: function(){
						...
						if (vid) {
								...
								if(!id || !vid.includes(id.toString()))
									$this.find('[name="id"]').attr('vid',parseInt(vid.split(',')[0]))[0].dispatchEvent(new Event('swatch'));
						}
			    },
					slideChangeTransitionEnd: function(){
						$thumb.update();
			...
			slide.removeClass('lh-reinit')
		}

	  setTimeout(function() {
	      jQuery.fn.zoom && $this.find('.zoom__image img:not(.zoomImg)').each(function() {
	          var $_this = jQuery(this);
	          $_this.trigger('zoom.destroy');
	          $_this.closest('.zoom__image').zoom({
	              url: $_this.attr('data-bigimg'),
	              touch: false
	          });
	      });
	  }, 0);
	...
	function filterSlide(filter){
		var all_slide = $this.find('.block-slide-lg,.block-slide-sm');
		all_slide.each(function(index, element){
			var slide_this  = $(this),
					data_this 	= arr_slide[index],
					html_filter = data_this.filter('[data-filter="'+filter+'"]');

			if(html_filter.length){
				slide_this[0].swiper && slide_this[0].swiper.destroy();
				slide_this.removeClass('.swiper-container-thumbs');
				slide_this.addClass('lh-reinit').removeClass('lh-showall').find('.swiper-wrapper').html(html_filter);
				$this.removeClass('lh-onchange-variant');
			}
			else{
				if(!slide_this.hasClass('lh-showall')){
					slide_this[0].swiper && slide_this[0].swiper.destroy();
					slide_this.removeClass('.swiper-container-thumbs');
					slide_this.addClass('lh-reinit').addClass('lh-showall').find('.swiper-wrapper').html(data_this);
				}
	...
	getDataSlide();
	filterSlide(_this.find('.lh-swatch-images .lh-swatch-select').data('value'));
	initSlide();
	ProductOptionsInit();

 };						cb.bind($('section[data-section-id="p0zzqrkw91g"] include[data-name="product-form"][data-id="4"]').get(0))(jQuery);;var cb  = function($) {
  var $this = jQuery(this);
  function eventMobile(){
    if($(window).width() < 768){
      $this.find('.lh-tab-contents .lh-item-tabs').each(function(){
        var __this = $(this);
        if(__this.hasClass('active'))
          __this.find('.lh-wrap-main').slideDown(100);
        else
          __this.find('.lh-wrap-main').slideUp(100);
      });
      $this.find('.lh-wrap-title').off('click','a');
      $this.find('.lh-wrap-title').on('click','a',function(event){
         event.preventDefault();
         var _this  = jQuery(this),
            _parent = _this.closest('.lh-item-tabs');
         _parent.toggleClass('active');
         _parent.find('.lh-wrap-main').slideToggle(200);
       });
    }else{
      $this.find('.lh-wrap-title').off('click','a');
      $this.find('.lh-tab-contents .lh-item-tabs').each(function(){
        ...
        $(this).find('.lh-wrap-main').slideDown(200);
      });
    }
 };
 $this.find('.lh-item-faq').each(function(){
...
if ($review && (window.LAYOUTHUB_LIVE || window.location.protocol === 'blob:')) {
  ...
  if (pid && isNaN(parseInt($review.data('pid')))) {
 ...
 };						cb.bind($('section[data-section-id="p0zzqrkw91g"] include[data-name="tabs"][data-id="5"]').get(0))(jQuery);;var cb  = function($) {
	if(document.querySelector('.page-container') && window.getComputedStyle(document.querySelector('.page-container')).transform.indexOf('matrix') != -1){
		document.querySelector('.page-container').style.webkitTransform='none';
		document.querySelector('.page-container').style.transform='none';
	}
	var _this = jQuery(this);
	_this.find('.lh-wrap-product-rating').click(function(){
		var tab_review = _this.find('.lh-item-tabs[data-type="review"]');
		if(tab_review.length){
			if($(window).width() < 768 && !tab_review.hasClass('active')){
				tab_review.addClass('active').find('.lh-wrap-main').slideDown(0);
			}
			tab_review[0].scrollIntoView({
			    behavior: "smooth",
			    block: "start"
 ...
 };						cb.bind($('section[data-section-id="p0zzqrkw91g"]').get(0))(jQuery);};					cb.bind($('section[data-section-id="p0zzqrkw91g"]').get(0))({						url: "https://library.layouthub.com/HUB/files/Tm92LS0yMDE5/NTc0ODY0ODc5/First_release/pages/Supe-Detail/product/",						settings: {"trans_unavai":"Unavailable","label_progress":"{left} items left","trans_outstock":"Out of stock"}					}, jQuery);})(jQuery);;console.log('This page layout has been built by https://www.layouthub.com');